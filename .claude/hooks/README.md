...
- Codex / 他の AI agent はこれらを実行しないため、hook が課す制約や挙動は必ず [`AGENTS.md`](../../AGENTS.md) の `Working Agreement` か [`.claude/rules/`](../rules/) に同等の規約として書くこと。
- そうすることで、Claude が hook で自動化する内容と Codex が手動で守るべき内容が一致する。

## 現状

- 現在は実行スクリプトを持たない。`AGENTS.md` と `.claude/rules/` の `@` import で同等の文脈を提供しているため、UserPromptSubmit での context 注入は冗長になる。
