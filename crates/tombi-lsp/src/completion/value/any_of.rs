...
use tombi_schema_store::{
    Accessor, AnyOfSchema, CurrentSchema, ReferableValueSchemas, SchemaAccessor, ValueSchema,
};
    ...
    T: FindCompletionContents + tombi_validator::Validate + Sync + Send + std::fmt::Debug,
{
    ...
    async move {
        ...
        // If every branch is resolved, keep a read lock; otherwise resolve refs with a write lock
        let resolved_only = {
            let schemas_read = any_of_schema.schemas.read().await;
