...
def add_label_to_image(
    ...
    # Position: adjusted position (up 20px, right 50px from previous)
...
def main():
    public_dir = Path("/Users/s23467/develop/tombi/docs/public")
    ...
    print(
        "Creating GIF animations with bold red Before/After labels (adjusted position)..."
    )
    print()

    success_count = 0
    for before, after, output in image_pairs:
        before_path = public_dir / before
        after_path = public_dir / after
        output_path = public_dir / output
    ...
    print()
    print(
        f"Successfully created {success_count}/{len(image_pairs)} GIF files with bold red labels."
    )
