...
Tombi の各データ構造は、それぞれ異なる目的に特化して設計されています：

1. **Token[]**: テキストの最小単位。字句解析の結果
2. **AST**: 構文構造を保持。位置情報を含み、テキスト編集に最適
3. **DocumentTree**: JSON に近い構造。位置情報を保持しつつスキーマ検証が可能。不完全な状態も表現可能
4. **Document**: 純粋なデータ。位置情報を持たず、Serialize/Deserialize に最適

各機能は目的に応じて適切なデータ構造を選択することで、効率的な実装を実現しています。
