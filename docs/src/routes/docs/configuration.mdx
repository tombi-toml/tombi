...
3. `.config/tombi.toml` in the current directory
4. `[tool.tombi]` in `pyproject.toml` in the current directory
5. ...search parent directory recursively
6. `$XDG_CONFIG_HOME/tombi/config.toml`
7. `~/.config/tombi/config.toml`
8. `~/Library/Application Support/tombi/config.toml` (macOS), `%APPDATA%\tombi\config.toml` (Windows)
9. `/etc/tombi/config.toml` (Linux)
10. editor settings via [workspace/didChangeConfiguration](https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#workspace_didChangeConfiguration) (LSP only)
