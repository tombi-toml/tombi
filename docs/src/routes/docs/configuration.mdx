...
If an extension or feature is omitted, it uses that setting's documented default value.
Most extension and LSP feature groups default to `true`, while some nested document-link options default to `false`.
...
### extensions."tombi-toml/cargo".lsp.document-link.workspace

Configure document links to workspace-managed Cargo dependencies and package metadata.
These links open workspace manifests referenced by `workspace = true` entries.

See [Cargo Extension > Document Links](/docs/extensions/tombi-extension-cargo#document-links).

- Type: `Table`

### extensions."tombi-toml/cargo".lsp.document-link.workspace.enabled

Enable or disable document links to workspace-managed Cargo references.
