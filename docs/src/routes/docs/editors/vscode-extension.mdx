...
|-------|------------------|--------------------------------------------------------------------------------------------|
| 1     | editor settings  | Path explicitly specified by `tombi.path` in the User `settings.json`                       |
...
The `tombi.path`, `tombi.args`, and `tombi.env` settings control how the Language Server executable is launched, so they are machine-scoped and can only be set in the User `settings.json`. Workspace `.vscode/settings.json` cannot override them, which prevents an untrusted workspace from hijacking the executable.
...
You can set offline mode in the User `settings.json`.
...
## Verbose Logging
You can enable verbose logging in the User `settings.json`.
