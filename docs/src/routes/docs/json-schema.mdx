...
### How to Clear Cache
#### CLI
Use `--no-cache` option to disable caching for a CLI command.
...
<Note>
The `--no-cache` option fetches the latest schema over the network without using the cache.
It only applies to catalogs/schemas used by the command, and does not update other file caches.
</Note>
...
<Note>
It removes **all** file caches and fetches the latest schema over the network.
</Note>

### Cache Location Search Priority

1. `$XDG_CACHE_HOME/tombi/cache.json`
2. `~/.cache/tombi/cache.json`
