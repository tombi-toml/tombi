...
<Note>
Tombi also accepts some non-standard compatibility keywords outside the dialect that defines them.
For example, `definitions` is still loaded alongside `$defs` in draft-2019-09 and draft-2020-12,
`dependencies` is still honored in draft-2020-12, and `prefixItems` is accepted even before
draft-2020-12.
</Note>
