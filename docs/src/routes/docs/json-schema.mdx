...
<img src="/x-tombi-additional-key-label.png" class="w-full" />
...
### Validation Score
For complex JSON Schemas, validation failures can produce many error messages.
To help users identify the most relevant errors,
Tombi uses a scoring system to filter out less important validation failures.
...
To disable strict mode globally, add `schema.strict = false` to your `tombi.toml` configuration.
You can also set `strict` on a matching `[[schemas]]` root or sub-schema entry.

Tombi resolves strict mode in this order:

1. The document comment directive: `#:tombi schema.strict = true` or `false`
2. The matching sub-schema `strict` setting
3. The matching root-schema `strict` setting
4. The global `schema.strict` setting
5. The built-in default, `true`

An omitted level falls through to the next one. A sub-schema setting applies only at and below its configured `root`.
...
You can customize the cache expiration time using the `TOMBI_CACHE_TTL` environment variable. See [Environment Variables](/docs/environment-variables#tombi-cache-ttl) for details.
...
implements the keyword when that dialect defines it.
...
<Note>
Tombi also accepts some legacy keywords for compatibility even when they are not part of the
selected dialect. For example, `definitions` is still loaded alongside `$defs` in draft-2019-09
and draft-2020-12, `dependencies` is still honored in draft-2020-12, and `prefixItems` is
accepted even before draft-2020-12.
</Note>
