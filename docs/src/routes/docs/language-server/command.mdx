...
- `schema`: Root schema assigned to the document (if available)
  - `uri`: Schema URI
- `subSchemas`: Sub-schemas mapped to specific roots (if available)
  - `root`: Schema access path (string form, e.g., `tool.tombi`)
  - `uri`: Schema URI
