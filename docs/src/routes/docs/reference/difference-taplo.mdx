...
In Taplo, you can use `include`/`exclude` patterns and `keys` to override formatting and schema settings for specific files or specific parts of TOML documents.
Similarly, Tombi's `[[overrides]]` allows you to override settings on a per-file basis using `files.include`/`files.exclude` patterns.

However, Tombi does not currently support the equivalent of Taplo's `keys` feature (overriding settings for specific key paths within TOML documents).
