...
### 5. Dangling comment の扱い
- **dangling comment は「次のグループ」に紐付ける**
- 紐付け先が明確でないコメントは、**直後のグループ先頭ノードの前**に置く
- `Separator` が存在する場合は、**Separator の直後**に移動してグループへ吸着
- これにより「グループに対する意図 (先頭コメント)」を表現できる
...
- 優先順位 (上から順に適用):
  ...
  3. **dangling**: グループ先頭ノードに作用 (グループ境界の外には出さない)
