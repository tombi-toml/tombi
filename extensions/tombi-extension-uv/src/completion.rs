...
use tombi_pep508::{AstNode, SyntaxKind};
...
async fn complete_package_spec(
    ...
    let relative_position = tombi_text::Position::from(position - dep_spec_range.start);

    // Parse the dependency specification to AST
    let parser = tombi_pep508::Parser::new(dep_spec.value());
    let (ast_root, _) = parser.parse_ast();

    // Get root node from AST
    let Some(root) = tombi_pep508::ast::Root::cast(ast_root.clone()) else {
        return Ok(None);
    };
    ...
    let Some(package_name) = requirement.package_name() else {
        return Ok(None);
    };

    tracing::info!(
        "Completion for '{}' at rel pos {:?}, package_name: {:?}",
        dep_spec.value(),
        position,
        package_name.name()
    );

    // Get token at cursor position - calculate byte offset from position
    let text_before_cursor = &dep_spec.value()[..dep_spec.value().len().min(relative_position.column as usize)];
    let byte_offset = text_before_cursor.len() as u32;
    let token_at_cursor = ast_root.token_at_offset(tombi_text::Offset::new(byte_offset));
    ...
    if relative_position > name_range.end {
        ...
        if let Some(ref extras) = extras_list {
            ...
            if extras_range.contains(relative_position) {
                ...
                let package_name_str = package_name.syntax().text().to_string();
            ...
            if version_range.contains(relative_position) {
                ...
                if has_operator {
                    ...
                    if relative_position > op_end {
                        ...
                        let package_name_str = package_name.syntax().text().to_string();
                        if let Some(mut versions) = fetch_package_versions(&package_name_str).await
                        {
