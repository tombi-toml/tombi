...
- [x] ルートが boolean schema（`true` / `false`）の JSON Schema を受理する（2026-03-08 対応）
- [x] サブスキーマ位置でも boolean schema を受理する（`items`, `properties`, `patternProperties`, `additionalProperties` など）（2026-03-08 対応）
...
- [x] `contentEncoding` / `contentMediaType` / `contentSchema` を annotation として扱う（2026-03-07 対応）
...
- [x] `unevaluatedProperties` / `unevaluatedItems` を実装する（2026-03-09 対応）
...
- `format` サポート拡張、content 系 annotation の保持
