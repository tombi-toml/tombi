...
pub fn run() -> Result<(), anyhow::Error> {
    ...
    write_file(
        ...
        &project_root_path().join("crates/tombi-ast-syntax/src/generated/syntax_kind.rs"),
    );

    write_file(
        &generate_ast_node(&ast)
            .context("Failed to generate ast node from grammar.".to_string())?,
        &project_root_path().join("crates/tombi-ast-syntax/src/ast/generated/ast_node.rs"),
    );

    write_file(
        &generate_ast_token().context("Failed to generate ast node from grammar.".to_string())?,
        &project_root_path().join("crates/tombi-ast-syntax/src/ast/generated/ast_token.rs"),
    );
